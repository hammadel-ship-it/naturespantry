...
const CSS = `@keyframes fadeUp    { from{opacity:0;transform:translateY(18px);}to{opacity:1;transform:translateY(0);} } @keyframes fadeIn    { from{opacity:0;}to{opacity:1;} } @keyframes spin      { to{transform:rotate(360deg);} } @keyframes pulse     { 0%,100%{opacity:.5;}50%{opacity:1;} } @keyframes float     { 0%,100%{transform:translateY(0);}50%{transform:translateY(-6px);} } @keyframes slideUp   { from{opacity:0;transform:translateY(20px);}to{opacity:1;transform:translateY(0);} } @keyframes pulseRing { 0%{box-shadow:0 0 0 0 rgba(34,163,90,.4);}70%{box-shadow:0 0 0 10px rgba(34,163,90,0);}100%{box-shadow:0 0 0 0 rgba(34,163,90,0);} } *{box-sizing:border-box;margin:0;padding:0;} html,body{height:100%;background:#0b1a0d;} body{font-family:'Georgia',serif;color:#e0ede2;-webkit-font-smoothing:antialiased;} input,textarea,button{font-family:'Georgia',serif;} .item-card:hover    {transform:translateY(-3px);box-shadow:0 6px 20px rgba(0,0,0,.2);} .rc:hover           {border-color:rgba(34,163,90,.4)!important;} .tier-card          {transition:transform .25s,box-shadow .25s;} .tier-card:hover    {transform:translateY(-6px);box-shadow:0 16px 48px rgba(0,0,0,.3);} .cta-btn            {position:relative;overflow:hidden;transition:transform .15s,box-shadow .15s;} .cta-btn:hover      {transform:scale(1.02);} .cta-btn:active     {transform:scale(.98);} .day-card           {transition:all .18s;cursor:pointer;} .day-card:hover     {transform:translateY(-2px);} .modal-wrap         {animation:fadeIn .2s ease;} .modal-box          {animation:slideUp .25s ease;} .search-ring:focus-within{border-color:rgba(80,180,100,.6)!important;box-shadow:0 0 0 3px rgba(34,163,90,.12)!important;} .pillar-tab         {transition:all .18s;cursor:pointer;} .chip-btn:hover     {filter:brightness(1.15);} @media(max-width:480px){ .np-week-grid{grid-template-columns:repeat(4,1fr)!important;} .np-item-grid{grid-template-columns:repeat(2,1fr)!important;} .np-tier-grid{grid-template-columns:1fr!important;} .np-modal-pad{padding:22px 16px!important;} .np-chips-wrap{grid-template-columns:repeat(2,1fr)!important;} } @media(min-width:481px) and (max-width:680px){ .np-week-grid{grid-template-columns:repeat(4,1fr)!important;} .np-item-grid{grid-template-columns:repeat(3,1fr)!important;} .np-tier-grid{grid-template-columns:1fr!important;} .np-chips-wrap{grid-template-columns:repeat(4,1fr)!important;} } @media(min-width:681px){ .np-week-grid{grid-template-columns:repeat(7,1fr)!important;} .np-item-grid{grid-template-columns:repeat(auto-fill,minmax(130px,1fr))!important;} .np-chips-wrap{grid-template-columns:repeat(4,1fr)!important;} }`;
...
s=s.replace(/”([^”]*)”/g,(*,inner)=>’”’+inner.replace(/\n/g,” “).replace(/\r/g,””)+’”’);
...
s=s.replace(/:\s*”(.*?)(?<!\)”/gs,(*,inner)=>’:”’+inner.replace(/(?<!\)”/g,’\”’)+’”’);
try{return JSON.parse(s);}catch(*){}
...
if(!res.ok)return;
let data; try { data=JSON.parse(await res.text()); } catch(*){return;}
const raw=(data.content||[]).map(b=>b.text||””).join(””).trim();
